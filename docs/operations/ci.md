...
| `auth-manager-integration` | auth-manager's real delegated access transport, from its `main` branch, can read, update and provision workspace access in this commit's e-sign over HTTPS, each app on its own database in one disposable MySQL 8.4 container. See [delegated-access.md](delegated-access.md). | `backend` changed |
...
| `image` | The production Docker image builds, serves `/up`, passes its own `HEALTHCHECK`, passes `nginx -t`, keeps credential-bearing signing and invitation paths out of its access log while still logging the session routes, and dispatches the `artisan`/`worker` roles correctly. | `backend`, `frontend`, or `docker` changed |
