...
| **A host that cannot start a child process** | Every read of an uploaded PDF runs in a child process with a hard `memory_limit` and a wall-clock deadline where the host can start one (`docs/adr/0006-process-bounded-document-reads.md`), so a document that costs more than its allowance is stopped by the runtime, not only by code remembering to report its work. `esign:doctor` reports which mode reads actually get. | **Where `ESIGN_DOCUMENTS_ISOLATION=auto` falls back to in-process, or `in_process` is set, the bound is cooperative only**: the configured ceilings trip for the work that reports to them, and a loop that does not report — or one long native call, which nothing inside PHP can interrupt — runs until PHP's own `memory_limit` or `max_execution_time`, which ends the request as a server error rather than a named refusal. A deployment that needs the hard bound sets `ESIGN_DOCUMENTS_ISOLATION=process`, which refuses to read documents rather than read them without it. |
| **Operator retains web access logs** | Signing pages send `Referrer-Policy: no-referrer` and load nothing from another origin. | The invitation credential is in the URL path. The Docker image's nginx does not log the token-bearing signing paths (`/sign/{envelope}/{token}` and its `/start`) or workspace invitation links (`/invitations/{token}`). Any other web server or reverse proxy in front of the application, including the cPanel host's own web server, still logs them unless the operator excludes them; an operator retaining those logs retains live credentials for their TTL. The application itself cannot keep a URL out of a proxy's log. |
