...
| `document_unavailable` | 503 | A document could not be read because of a failure on the service side — for example while resolving anchors at send. Nothing about the request needs to change, nothing changed, and `details.retryable` is `true` |
