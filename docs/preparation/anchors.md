...
`PreflightLimits` the upload was inspected under, and every phase of reading the document is charged
to that one budget, through the reporting described in [documents.md](documents.md):
...
document whose pages cannot be read: that will not read on retry, so it is not reported as the
retryable `AnchorDocumentUnavailable`, which stays for storage failures and bytes that do not hash
to their row.
