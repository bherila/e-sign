...
service**, because the precision rule cannot be expressed in JSON Schema at all.
...
it does it says which option and why — `coordinate_too_precise` — rather than reporting it as
malformed. The difference matters to the person on the other end: a sender told "invalid" changes
