...
| Scope for template reads | not upstream — upstream keys are workspace-wide | **intentionally different** | `FirmaAuthTest::test_building_from_a_template_requires_the_template_scope_on_both_routes`, `::test_a_document_only_create_does_not_need_the_template_scope` | `templates:read` is required by any create that supplies `template_id`, on **either** create route, and is enforced where the condition is known rather than named on a route. Both routes accept either a template or an inline document, so naming it on one left the gate bypassable through the other — the same agreement could be built from the same template and then sent — while naming it on both would demand a template scope of a caller that only ever posts its own PDFs. |
...
| Error envelope | `Error` = `{error (required, human-readable), message, details}` | supported | `FirmaAuthTest` (401/403/404/501), `FirmaLifecycleTest` (409/422), `FirmaCreateAndSendTest` (400) | `error` carries a short machine token and `message` the sentence, which is what every recorded upstream example does even though the schema calls `error` human-readable. `details` appears only when there is something structured to say. `create-and-send` keeps its own envelope; see D1. A storage failure is deliberately **not** translated: its message carries the disk name and the object path, so it falls through to a fixed sentence and a reported `500` rather than a `400` that would leak a storage key and have the caller retry a body that was never wrong (`FirmaCreateAndSendTest::test_a_storage_failure_is_an_internal_error_that_leaks_nothing`). |
...
| `template_id` namespace | `format: uuid`, upstream's own id | supported | `FirmaLifecycleTest::test_a_draft_is_created_from_an_imported_template_alias`, `::test_the_created_template_id_round_trips`, `::test_a_template_version_id_also_resolves` | Accepts this application's `templates.public_id`, an imported alias from `template_aliases` — the provider id the consumer already has hardcoded — or a template *version* public id. Native id first, because that namespace is ours. `docs/HANDOFF.md` §6 keeps the first two in separate fields, so neither ever becomes the other. The create response echoes the **template's** id rather than the version the envelope copied, so the one id a consumer would store is one the next create accepts; the version form resolves as well, for anything an integration read from elsewhere. |
...
| Atomicity | `500` description: "Any partially-created signing request is rolled back, so no draft remains." | supported | `FirmaCreateAndSendTest::test_hand_drawn_only_is_not_implemented`, `::test_a_coordinate_outside_the_percent_range_is_refused`, `::test_an_anchor_that_matches_nothing_is_refused` | Matches our fail-closed rule and is stricter: everything judgeable without the document — an unsupported option, an out-of-range percentage, an unplaceable field type, a recipient with no `order`, an approver — is refused *before* the bytes are stored, so those requests leave nothing at all. A refusal that genuinely needed the document leaves the document, with its preflight report, because keeping an upload is intake's own rule — that report is the only evidence of what was uploaded when a sender reports a rejection. No signing request is ever half-created either way. The consumer's platform-NDA path currently warns and returns a link on send failure; the migration must fail closed instead (`docs/HANDOFF.md` §2). |
...
| Anchors | `fields[].anchor` is not upstream's shape; upstream uses a separate `anchor_tags[]` collection with `offset_units: percent\|pixels` and its own type enum | **intentionally different** | `FirmaCreateAndSendTest::test_an_anchored_field_is_placed_from_the_document_text` (which also asserts the stored receipt), `::test_an_anchor_that_matches_nothing_is_refused`, `::test_an_anchor_offset_that_leaves_the_page_is_refused`, `::test_a_non_numeric_anchor_offset_is_refused_rather_than_coerced`, `EnvelopeAnchorResolutionTest::test_a_schema_that_arrives_resolved_is_resolved_again_to_the_same_bytes` | `fields[].anchor = {text, occurrence?, origin?, offset_x?, offset_y?}` is a documented compatibility extension. The text is located by `PdfTextLocator` — a real content-stream parse — and resolved by `AnchorResolver`. Offsets are percentages of the page, like every other number in this profile, in `-100..100` because an offset is a displacement; a second coordinate system with its own unit switch is how a signature ends up in the wrong place, which is why `anchor_tags[]` itself is `501`. `position.width` and `position.height` are still required, because an anchor says where a field goes and never how big it is. A non-numeric offset is a `400` rather than a silent zero, and the **resolved** rectangle is checked against the page: an anchored field is built from where the text turned out to be, so nothing before resolution knows whether it fits, and the native schema validator's page-fit rule is inert on this path because `FieldSchemaDocument::fromArray()` supplies no `PageSizes`. An anchor that matches nothing, or ambiguously, is a `400`. Since issue #23 the emitted native field carries the anchor **and its receipt** — `placement: replace`, `required: true`, the origin and offset converted to points, and `anchor.resolved` naming the revision digest the text was located in. The receipt is a record of what was found, not a shortcut: the envelope's own send-time resolution measures again regardless, on the same bytes, and stores the same rectangle. Two consequences worth planning for. The stored `anchor.text` is the **normalised** request the facade actually resolved, not the caller's string verbatim, so both passes ask the same question. And a *standalone* `POST /signing-requests/{id}/send` reads the document, so a storage outage fails it with a retryable `document_unavailable` even though the coordinates are already recorded; `create-and-send` does not, because `Documents\RevisionBytes` carries the bytes it proved during creation through to the send in the same request rather than opening the object twice with a window in between. `anchor.required: false` is not reachable from this profile: upstream has no way to say an anchor may be absent, and inventing one here would be an extension nobody asked for. See `docs/preparation/anchors.md`. |
