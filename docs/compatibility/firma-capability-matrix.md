...
2. **No invented facts.** `credit_cost`, `credits_remaining`, `first_name`/`last_name`, the
   contact block, the corner positions, and ten of the fourteen `settings` members are null or
   false with a documented reason, because a plausible value for something this product does
...
| Base URL / signing links | Consumer hardcodes `https://app.firma.dev/signing/{recipientId}`; upstream returns `CreateAndSendResponse.first_signer.signing_link` as an opaque `uri` | **intentionally different** | `FirmaCreateAndSendTest::test_the_document_is_ingested_and_the_request_is_sent` | We do not impersonate the Firma hosted site (`docs/HANDOFF.md` §10). Our signing UI is served from this application and visibly branded as this product. The consumer must stop constructing the URL and read `signing_link`, which is minted by the same `SigningUrlMinter` the invitation email uses — so the link in the response and the link in the signer's inbox are one session, not two. |
...
| `settings` | `SigningRequestSettings`, 14 members | **intentionally different** | idem | Every member emitted so the key set matches. Two describe something this product models — `use_signing_order` (sequential vs parallel) and `allow_download` (always true; every party can fetch the executed PDF through an authorized download). The other twelve report `false` or `null`, each documented in `SigningRequestSettings` with which it is and why. `null` is used where upstream's own recorded value is null; inventing `true` for a setting nothing enforces would be a successful no-op one read at a time. Asking for one of them is a different matter — see the `501` rows. |
