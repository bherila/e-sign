...
| Leaking the token through `Referer` | `Referrer-Policy: no-referrer`, and no third-party origin to leak to. | **The token is in a URL path.** The Docker image's nginx does not write `/sign/{envelope}/{token}` or its `/start` to the access log. Any other web server or reverse proxy in front, including the cPanel host's own web server, still does unless the operator excludes those paths; operators who retain such logs retain live credentials for their TTL. The application cannot fix that for a proxy it does not ship. |
