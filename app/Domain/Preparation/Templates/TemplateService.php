...
final readonly class TemplateService
{
     ...
     *
     * Publishing is the last moment a template can be fixed cheaply, and the first moment a field
     * set and the exact bytes it will be placed on are both fixed: a version snapshots one
     * immutable review revision, and publishing freezes the field set against it. So an anchor
     * whose text is not in that document, or is in it twice, is discoverable *now* — while the
     * sender is still authoring — instead of when they try to send.
     *
     ...
     * @throws AnchorDocumentUnavailable When the revision's bytes cannot be read: a retryable
     *                                   server failure, never a problem with the field set.
     * @throws AnchorResolutionDefect When resolution contradicted its own request: a defect in
     *                                this service, never in the document.
     */
    public function publish(TemplateVersion $version, User $actor): TemplateVersion
    {
        ...
        // Cheap refusal first. A repeat request against a published version would otherwise pay
        // for a document read and a full parse before the locked check told the caller what it
        // already knew. The authoritative check is still the one under the lock.
        if ($version->isPublished()) {
            throw TemplateStateException::versionAlreadyPublished($version->public_id);
        }

        // Outside the transaction: resolving reads a document object and parses its content
        // streams, and doing that under `lockForUpdate()` would hold the template-version row for
        // the length of a PDF parse. Safe out here because the result is confirmed against the
        // locked row below, and a draft nobody else is editing simply matches.
        ...
        return DB::transaction(function () use ($version, $actor, $template, $prepared): TemplateVersion {
            ...
            // The same save that stamps `published_at` stores the placed field set: the model's
            // immutability guard reads the *original* value, so this write is the last one allowed.
     ...
     *
     * {@see publish()} resolves before opening the transaction so a PDF parse does not happen under
     * the row lock. A draft can still be edited in between, so the digest of the field set the pass
     * ran against is compared with the locked row's, and a disagreement resolves again — inside
     * the lock this time, because correctness outranks the lock-duration saving and a racing edit is
     * rare.
     *
     * @throws InvalidFieldSchemaException
     * @throws AnchorDocumentUnavailable
     * @throws AnchorResolutionDefect
     */
     ...
     *
     * Absent optional anchors are reported, not applied: see {@see publish()}. A field set that
     * cannot be placed becomes an {@see InvalidFieldSchemaException}, so publishing answers exactly
     * like drafting does — a 422 carrying every problem at once, each with a stable code and a JSON
     * Pointer to the offending field. The two server failures leave as themselves: neither is
     * something the sender can fix by editing the document.
     *
     * @throws InvalidFieldSchemaException
     * @throws AnchorDocumentUnavailable
     * @throws AnchorResolutionDefect
