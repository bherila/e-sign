...
final readonly class SchemaAnchorResolver
{
    ...
    public function resolve(
        ...
        foreach ($schema->fields as $index => $field) {
            ...
            // The search is charged inside matching, in the units it multiplies — candidate runs,
            // bytes searched, matches found — so one field over a large run set is bounded while
            // it is searched, not only before it starts. A budget looked at once per field, before
            // the scan, let that scan run to completion unmetered.
            $outcome = $this->resolveField($index, $field, $anchor, $runs, $documentSha256, $pageSizes, $budget);

            // One field finished is one unit, reported after it.
            $budget?->step();
    ...
    private function resolveField(
        ...
        try {
            $matches = $this->anchors->resolve($runs, $request, $budget);
