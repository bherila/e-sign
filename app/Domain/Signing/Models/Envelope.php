 ...
 * statement.
 *
 * There is exactly one thing the state machine writes through that door: `send()` stores the
 * anchor-resolved `field_schema` and its digest in the same statement as the transition to
 * `sent`. That is not an edit of the snapshot but its completion. An anchored field arrives
 * carrying a question — "put this box next to the words `Signature:`" — and resolution is where
 * that becomes a coordinate. It happens before anybody is invited, so nothing has been shown for
 * assent yet. Afterwards the rule holds without an exception: nothing re-resolves, and a rectangle
 * a signer saw can never move (docs/preparation/anchors.md).
 *
 * @property int $id
...
class Envelope extends Model
{
     ...
     *
     * The copied schema is immutable for the whole life a signer can see, but `send()` writes the
     * anchor-resolved schema through the query builder and then refreshes, and a stale decode would
     * hand the rest of that transition the pre-resolution field set. `setRawAttributes()` is the
     * one door every refresh goes through, so it is cleared there.
     */
    ...
    /**
     * Fields declared in the source but left out because an optional anchor was not in the document.
     *
     * Empty for almost every envelope. When it is not, each entry names the field, its recipient,
     * the anchor text that was looked for, and the reason — see
