...
/**
 * Send-time anchor resolution (issue #23, piece D).
 *
 * Send is the authoritative resolution, because an envelope does not have to come from a template:
 * the native API and the Firma facade both build one straight from a document, and neither has a
 * publish step to have caught anything at. What this file pins is that by the time anyone is
 * invited the envelope holds concrete rectangles, that an anchor which cannot be placed stops the
 * send with a message naming the field, that a document which cannot be read is a server failure
 * rather than a problem with the field set, and that after the send nothing looks at the document's
 * text again.
 *
 ...
 */
final class EnvelopeAnchorResolutionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_a_cross_check_that_disagrees_with_the_declared_rectangle_stops_the_send(): void
    {
        ...
        $failure = $this->refusedSend($this->scenario->preparedDraft(['field_schema' => $schema]));

        $this->assertTrue($failure->hasCode(ValidationCode::AnchorCrossCheckFailed->value));
    }
    ...
    public function test_an_anchor_failure_is_reported_alongside_every_other_send_problem(): void
    {
        ...
        // Send is the last cheap moment, so the sender sees the whole list at once.
    ...
    public function test_an_absent_optional_anchor_omits_the_field_and_records_it_on_the_envelope(): void
    {
        ...
        // Key-sorted on both sides: a MySQL JSON column does not preserve object key order.
    ...
    public function test_a_value_supplied_for_an_omitted_field_is_discarded_with_it(): void
    {
        ...
        // The sender prefilled the optional field before send; its anchor then turns out not to be
        // in the document.
        ...
        // row it finds, and a value for a field the agreement does not contain would appear in the
        // evidence as an untyped stray.
    ...
    public function test_the_omission_reaches_the_audit_trail(): void
    {
        ...
        $payload = AuditEvent::query()->where('action', EnvelopeEvent::Sent->value)->sole()->payload;
     ...
     *
     * Resolution is a pure function of the request and the bytes, so measuring an unchanged pair a
     * second time returns the first answer exactly — including the digest the receipt names.
     */
     ...
     * origin corner or the offset. Were a receipt honoured, a required anchor whose text is not in
     * the document would sail through the one check that exists to catch it.
     */
    public function test_a_receipt_does_not_answer_an_anchor_whose_text_has_since_changed(): void
    {
        $schema = $this->preResolvedSchema();
        $index = array_search('seller_signature', array_column($schema['fields'], 'id'), true);
        $this->assertIsInt($index);

        $schema = SigningFixtures::mutateField($schema, 'seller_signature', [
            'anchor' => array_replace($schema['fields'][$index]['anchor'], ['text' => 'Nowhere in this document:']),
        ]);
    ...
    public function test_a_resolved_schema_still_needs_the_document_at_send(): void
    {
        ...
        // schema above are carried only as far as the request that proved them.
        ...
        try {
            $this->scenario->machine()->send($envelope);
            $this->fail('A schema carrying receipts was sent without reading its document.');
        } catch (AnchorDocumentUnavailable) {
        }
    ...
    /**
     * A document that cannot be read is not a field set that is wrong.
     */
    ...
    public function test_bytes_that_are_not_the_revision_are_refused_before_anything_is_measured(): void
    {
        ...
        // cannot see.
        Storage::disk('documents')->put($this->scenario->revision->path, PdfFixtures::bytes('multi-page-mixed-size'));

        try {
            $this->scenario->machine()->send($envelope);
            $this->fail('Anchors were measured in bytes that are not the envelope\'s revision.');
        } catch (AnchorDocumentUnavailable) {
        }
    ...
    /**
     * An envelope whose revision no longer carries the digest it was created against is a
     * data-integrity failure on the service side, not something the sender can fix.
     */
    public function test_a_revision_that_no_longer_matches_the_envelope_digest_is_a_server_failure(): void
    {
        $envelope = $this->scenario->preparedDraft(['field_schema' => $this->anchoredSchema()]);

        // The revision is internally consistent — its bytes match its own digest — but they are not
        // the bytes the envelope was created against. The byte check passes, so only the envelope
        // digest comparison stands between these anchors and a document nobody reviewed.
        $other = PdfFixtures::bytes('multi-page-mixed-size');
        Storage::disk('documents')->put($this->scenario->revision->path, $other);
        $this->scenario->revision->newQuery()->whereKey($this->scenario->revision->getKey())
            ->update(['sha256' => hash('sha256', $other)]);
        app()->forgetScopedInstances();

        try {
            $this->scenario->machine()->send($envelope);
            $this->fail('Anchors were resolved against a revision the envelope was not created against.');
        } catch (AnchorDocumentUnavailable) {
        }
    ...
    public function test_the_native_api_publishes_what_was_omitted(): void
    {
        ...
        // And it stays visible, long after the send.
     ...
     *
     * @return AnchorResolution&object{calls: int}
     */
    private function countingResolution(): AnchorResolution
    {
    ...
    /**
     * The sequential two-signer fixture with the counterparty's signature placed by anchor and the
     * optional notes field placed against the second "Notes:" on page 2.
     *
