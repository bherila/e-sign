...
final class SchemaAnchorResolverTest extends TestCase
{
    ...
    /**
     * Matching is charged while it scans, not once per field before the scan starts.
     *
     * One anchored field over a very large run set is a single filter, sort and substring scan. A
     * budget looked at once per field, before that scan, let it run to completion however long it
     * took. The clock here advances one second per look, so the time ceiling is a ceiling on
     * looks: fifty thousand filler runs — about 1.5 MB of text to search — must be looked at while
     * they are searched, and one field's worth of looks cannot reach a hundred.
     */
    public function test_matching_is_charged_while_it_scans(): void
    {
        $looks = 0;
        ...
        for ($i = 0; $i < 50_000; $i++) {
            $runs[] = new TextRun(
                1,
                'filler text that is not the anchor',
                new NativeRect(10.0, 700.0, 100.0, 10.0),
                10.0,
                'F1',
        ...
        $budget = new PreflightBudget(new PreflightLimits(timeBudgetSeconds: 100.0), $clock);

        try {
            $this->resolver()->resolve($this->documentWith($this->replacingAnchor()), $runs, self::DIGEST, null, true, $budget);
