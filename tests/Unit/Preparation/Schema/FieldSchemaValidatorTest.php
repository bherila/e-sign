...
class FieldSchemaValidatorTest extends TestCase
{
    ...
    /**
     * Every numeric property, probed at a magnitude where the two implementations part company.
     *
     * The table in `docs/preparation/field-schema.md` claims which numbers are bounded. It was
     * wrong once, and wrong in a way a reader could not see: it asserted a property of the
     * *contract* from the behaviour of one implementation. This case and its TypeScript twin
     * (`fieldSchema.test.ts`, "the numeric sweep") are the same list with the same expectations,
     * so a bound added on one side and not the other fails here rather than in review.
     *
     * 1e20 is the probe because that is where PHP's `json_encode()` starts writing `1.0e+20`
     * against JavaScript's `100000000000000000000`: a document holding one has two canonical
     * forms, and therefore two `field_schema_sha256`.
     *
     * @return iterable<string, array{callable(array<string, mixed>): array<string, mixed>, bool}>
     */
    public static function numericSweep(): iterable
    {
        $receipt = static fn (array $over = []): array => array_replace([
        ...
        yield 'fields[].page' => [static function (array $d): array {
            $d['fields'][0]['page'] = 1e20;

            return $d;
        }, true];
        yield 'fields[].rect.x' => [static function (array $d): array {
            $d['fields'][0]['rect']['x'] = 1e20;

            return $d;
        }, false];
        yield 'fields[].rect.width' => [static function (array $d): array {
            $d['fields'][0]['rect']['width'] = 1e20;

            return $d;
        }, false];
        yield 'anchor.occurrence' => [static function (array $d): array {
            $d['fields'][5]['anchor']['occurrence'] = 1e20;

            return $d;
        }, true];
        yield 'anchor.offset.dx' => [static function (array $d): array {
            $d['fields'][5]['anchor']['offset']['dx'] = 1e20;

            return $d;
        }, false];
        yield 'anchor.tolerance' => [static function (array $d): array {
            $d['fields'][5]['anchor']['placement'] = 'cross_check';
            $d['fields'][5]['anchor']['tolerance'] = 1e20;

            return $d;
        }, true];
        yield 'anchor.resolved.anchor_rect.x' => [static function (array $d) use ($receipt): array {
            $d['fields'][5]['anchor']['resolved'] = $receipt(['anchor_rect' => ['x' => 1e20, 'y' => 1, 'width' => 1, 'height' => 1]]);

            return $d;
        }, true];
        yield 'anchor.resolved.anchor_rect.width' => [static function (array $d) use ($receipt): array {
            $d['fields'][5]['anchor']['resolved'] = $receipt(['anchor_rect' => ['x' => 1, 'y' => 1, 'width' => 1e20, 'height' => 1]]);

            return $d;
        }, true];
        yield 'anchor.resolved.page' => [static function (array $d) use ($receipt): array {
            $d['fields'][5]['anchor']['resolved'] = $receipt(['page' => 1e20]);

            return $d;
        }, true];
        yield 'anchor.resolved.occurrence_index' => [static function (array $d) use ($receipt): array {
            $d['fields'][5]['anchor']['resolved'] = $receipt(['occurrence_index' => 1e20]);

            return $d;
        }, true];
    }

    #[DataProvider('numericSweep')]
    public function test_the_numeric_sweep_matches_the_documented_table(callable $mutate, bool $bounded): void
    {
        $refused = ! (new FieldSchemaValidator)->validate($mutate(FieldSchemaFixture::asArray()))->isValid();

        $this->assertSame(
            $bounded,
            $refused,
            $bounded
                ? 'This property is documented as bounded and must refuse 1e20.'
                : 'This property is documented as unbounded (#105); if that changed, update the table in '
                    .'docs/preparation/field-schema.md and its TypeScript twin.',
        );
    }
