...
6. **Keep production migrations backward-compatible with the previous release for at least one
   release.** Expand first: add nullable/defaulted columns or new tables, deploy code that can use
   both shapes, and backfill separately when needed. Remove old columns, tables, constraints, or
   compatibility code only in a later release after the previous application version no longer
   needs them. Deploy rollback selects the previous code release; it never runs
   `migrate:rollback` against production.
7. **Use `dateTime()` for NOT NULL date columns**, never a bare `timestamp()` without
...
8. Production is MySQL 8 or MariaDB. Migrations must be valid on SQLite and both engines; never
