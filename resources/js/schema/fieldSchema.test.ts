...
describe("the numeric sweep", () => {
  /**
   * The twin of `FieldSchemaValidatorTest::numericSweep`, with the same list and the same
   * expectations.
   *
   * The table in `docs/preparation/field-schema.md` says which numbers this schema bounds. It was
   * wrong once, and invisibly so: it asserted a property of the *contract* from one
   * implementation's behaviour. Running the same probes on both sides is what makes the table a
   * verified claim rather than an inferred one, and a bound added here and not in PHP — or the
   * reverse — fails instead of reaching an integration as a 422.
   *
   * 1e20 is the probe because that is where the two runtimes part company: PHP writes `1.0e+20`
   * where this one writes `100000000000000000000`, so a document holding one has two canonical
  ...
  it.each([
    ["fields[].page", (d: Record<string, any>) => (d.fields[0].page = 1e20), true],
    ["fields[].rect.x", (d: Record<string, any>) => (d.fields[0].rect.x = 1e20), false],
    ["fields[].rect.width", (d: Record<string, any>) => (d.fields[0].rect.width = 1e20), false],
    ["anchor.occurrence", (d: Record<string, any>) => (d.fields[5].anchor.occurrence = 1e20), true],
    ["anchor.offset.dx", (d: Record<string, any>) => (d.fields[5].anchor.offset.dx = 1e20), false],
    [
      "anchor.tolerance",
      (d: Record<string, any>) => {
    ...
    [
      "anchor.resolved.anchor_rect.x",
      (d: Record<string, any>) =>
        (d.fields[5].anchor.resolved = receipt({ anchor_rect: { x: 1e20, y: 1, width: 1, height: 1 } })),
      true,
    ],
    [
      "anchor.resolved.anchor_rect.width",
      (d: Record<string, any>) =>
        (d.fields[5].anchor.resolved = receipt({ anchor_rect: { x: 1, y: 1, width: 1e20, height: 1 } })),
      true,
    ],
    ["anchor.resolved.page", (d: Record<string, any>) => (d.fields[5].anchor.resolved = receipt({ page: 1e20 })), true],
    [
      "anchor.resolved.occurrence_index",
      (d: Record<string, any>) => (d.fields[5].anchor.resolved = receipt({ occurrence_index: 1e20 })),
      true,
    ],
  ])("%s is bounded as documented", (_name, mutate, bounded) => {
    const document = JSON.parse(JSON.stringify(fixtureJson)) as Record<string, any>;
    mutate(document);

    expect(validateFieldSchema(document).length > 0).toBe(bounded);
  });
});
